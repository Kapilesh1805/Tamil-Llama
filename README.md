...
This repository now also includes **TanglishBridge**, a lightweight inference-time pipeline for the research contribution:

> **TanglishBridge: Enabling Code-Mixed Tamil-English Interaction for Tamil-LLaMA without Fine-Tuning**

TanglishBridge sits **around** Tamil-LLaMA and improves handling of:

- Tanglish inputs such as `bro saptiya?`
- Romanized Tamil inputs such as `vanakkam, nee eppadi irukka?`
- Code-mixed Tamil-English inputs such as `naan office ku varen`

The approach avoids any model retraining and instead combines:

1. Script detection and Code-Mixing Index (CMI) estimation
2. Tanglish normalization
3. Smart Romanized Tamil transliteration
4. Response post-processing that matches the user's input style

Main entry points added in this repo:

- `tanglishbridge/` for the reusable pipeline package
- `run_pipeline.py` for quick CLI testing
- `app.py` for the Streamlit demo
- `scripts/eval/evaluate.py` for baseline-vs-TanglishBridge evaluation
- `scripts/eval/cmi_analysis.py` for dataset-level CMI analysis
- `scripts/eval/human_eval_template.py` and `scripts/eval/human_eval_analyze.py` for human studies
...
- [Demo](#demo)
- [TanglishBridge Quickstart](#tanglishbridge-quickstart)
...
TanglishBridge will try to load `abhinand/tamil-llama-7b-instruct-v0.1`. Export your token before running the pipeline:
...
### 3. Run the quick pipeline smoke test
...
### 5. Run automatic evaluation

```bash
python scripts/eval/evaluate.py
python scripts/eval/cmi_analysis.py
python scripts/eval/human_eval_template.py
```

The evaluation pipeline writes:

- `scripts/eval/evaluation_results.txt`
- `scripts/eval/metrics.json`
- `data/processed/cmi_analysis.json`
- `data/processed/cmi_histogram.png`
- `data/processed/cmi_piechart.png`
- `scripts/eval/human_eval_sheet.csv`

### TanglishBridge package overview

- `tanglishbridge/detector.py`: script detection, token language ID, CMI scoring
- `tanglishbridge/normalizer.py`: abbreviation expansion and Romanized Tamil verb normalization
- `tanglishbridge/transliterator.py`: Romanized Tamil to Tamil-script conversion with mixed-text awareness
- `tanglishbridge/postprocessor.py`: style-matched output cleanup
- `tanglishbridge/pipeline.py`: end-to-end orchestration over Tamil-LLaMA
